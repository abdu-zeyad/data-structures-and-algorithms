...
const toLastNames = (people) => {
  const usersByLikes = people.map((item) => {
    let container = {};
...
const addPurchases = (arr) => {
  // Solution code here...
  let x = arr.map((item) => item.purchasePrice).reduce(sum, 0);
  function sum(num, total) {
    total = num + total;
    return total;
  }
...
const countNumberOfElements = (arr) => {
  let count = arr.reduce((counter, obj) => {
